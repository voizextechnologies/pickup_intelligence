          ...
          <div className="mb-6">
            ...
            {expandedSections.owner && (
              <div className={`p-4 mt-2 rounded-lg border ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Client ID:</span>
                    <div className="flex items-center space-x-2">
                      <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        {searchResults['x-client-unique-id'] || 'N/A'}
                      </span>
                      {searchResults['x-client-unique-id'] && (
                        <button
                          onClick={() => copyToClipboard(searchResults['x-client-unique-id'])}
                          className="p-1 text-cyber-teal hover:text-electric-blue transition-colors"
...
<div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
  <button
    onClick={() => {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `vehicle-rc-${rcNumber}-${Date.now()}.json`;
      link.click();
      URL.revokeObjectURL(url);
      toast.success('Results exported successfully!');
    }}
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
            Credits Available: {officer.credits_remaining}
          </span>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex space-x-4 mb-6 border-b border-cyber-teal/20">
        <button
          onClick={() => setActiveTab('phone-prefill-v2')}
          className={`px-4 py-2 text-sm font-medium ${
            activeTab === 'phone-prefill-v2'
              ? 'border-b-2 border-cyber-teal text-cyber-teal'
              : isDark
              ? 'text-gray-400 hover:text-cyber-teal'
              : 'text-gray-600 hover:text-cyber-teal'
          }`}
        >
          Phone Prefill V2
        </button>
      ...
      {activeTab === 'imei' && (
        <div className={`border border-cyber-teal/20 rounded-lg p-6 ${
          ...
          <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            IMEI lookup functionality coming soon.
          </p>
        </div>
      )}
      {activeTab === 'fasttag' && (
        <div className={`border border-cyber-teal/20 rounded-lg p-6 ${
          ...
          <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Fastag lookup functionality coming soon.
          </p>
        </div>
      )}
      {activeTab === 'credit-history' && (
        <div className={`border border-cyber-teal/20 rounded-lg p-6 ${
          ...
          <div className="flex items-center space-x-3 mb-6">
            <CreditCard className="w-6 h-6 text-cyber-teal" />
            <h3 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              Credit History
            </h3>
          </div>
          <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Credit history lookup functionality coming soon.
          </p>
        </div>
      )}
      {activeTab === 'cell-id' && (
        <div className={`border border-cyber-teal/20 rounded-lg p-6 ${
          ...
          <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Cell ID lookup functionality coming soon.
          </p>
        </div>
      )}
    </div>
