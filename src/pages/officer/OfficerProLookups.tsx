...
import { Search, Phone, Mail, User, CheckCircle, XCircle, Zap } from 'lucide-react';
...
export const OfficerProLookups: React.FC = () => {
  ...
  const handleSearch = async (type: string) => {
    ...
    try {
      ...
      if (type === 'recharge-status') {
        // Send as form-urlencoded for PlanAPI
      ...
      if (type === 'recharge-status') {
        ...
        if (data && data.Status) {
          ...
          throw new Error('Invalid response format from Recharge API');
        }
      } else {
        // Handle LeakOSINT response structure
      ...
      await addQuery({
        ...
        result_summary: type === 'recharge-status' 
          ? `Recharge status check for ${inputData}` 
          : data && data["List"] && Object.keys(data["List"]).length > 0 
  ...
  const handleRechargeStatusCheck = async () => {
    await handleSearch('recharge-status');
  };

  const renderRechargeStatusCheck = () => (
    ...
    <div className={`p-6 space-y-6 min-h-screen ${isDark ? 'bg-crisp-black' : 'bg-soft-white'}`}>
      ...
      <div className={`border border-cyber-teal/20 rounded-lg p-4 shadow-lg ${
        ...
        <div>
          ...
          {activeTab === 'recharge-status' && renderRechargeStatusCheck()}
