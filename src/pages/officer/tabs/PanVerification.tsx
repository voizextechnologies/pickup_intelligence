...
const PanVerification: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<PanVerificationResult | null>(null);
  ...
  const handlePanVerification = async () => {
    ...
    if (!panAPI) {
      ...
      setSearchError('PAN Verification API not configured');
      return;
    }
    ...
    try {
      ...
      const data: PanVerificationResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `PAN Verification query for ${cleanPanNumber}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          source: 'PlanAPI',
          result_summary: `PAN verification for ${cleanPanNumber}`,
          full_result: data,
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('pan verification'))?.default_credit_charge || 5} credits per query.
      </p>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `pan-verification-${panNumber}-${Date.now()}.json`;
                link.click();
