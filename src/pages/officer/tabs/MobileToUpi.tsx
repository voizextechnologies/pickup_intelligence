...
import { Shield, AlertCircle, CheckCircle, ChevronDown, ChevronUp, Copy, Download, Search, Phone } from 'lucide-react';
import { useTheme } from '../../../contexts/ThemeContext';
import { useOfficerAuth } from '../../../contexts/OfficerAuthContext';
import { useSupabaseData } from '../../../hooks/useSupabaseData';
import toast from 'react-hot-toast';

interface MobileToUpiResult {
  [key: string]: any;
}
...
const MobileToUpi: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<MobileToUpiResult | null>(null);
...
const mobileAPI = enabledAPIs.find(api =>
  api.name.toLowerCase().includes('mobile to upi') && api.key_status === 'Active'
    ...
    if (!mobileAPI) {
      ...
      setSearchError('Mobile to UPI API not configured');
      return;
    }

    const creditCost = mobileAPI.default_credit_charge || 1.80;
    ...
    try {
      const apiParts = mobileAPI.api_key.split(':');
      ...
      const baseUrl = '/api/planapi/api/Ekyc/MobileNoToVPA';
      ...
      const data: MobileToUpiResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Mobile to UPI query for ${cleanMobileNo}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'Mobile to UPI',
          input_data: `Mobile No: ${cleanMobileNo}, ApiMode: 1`,
          source: 'RapidAPI',
          result_summary: `Mobile to UPI for ${cleanMobileNo}: ${data.Status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.Status === 'Success') {
        toast.success('Mobile to UPI mapping successful!');
      } else {
        toast.error(`Mapping failed: ${data.Message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Mobile to UPI error:', error);
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'Mobile to UPI',
          input_data: `Mobile No: ${mobileNo}, ApiMode: 1`,
          source: 'RapidAPI',
    ...
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="flex items-end">
          <button
            onClick={handleMobileToUpi}
            disabled={isSearching || !mobileNo.trim()}
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('mobile to upi'))?.default_credit_charge || 1.80} credits per query.
      </p>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `mobile-to-upi-${mobileNo}-${Date.now()}.json`;
                link.click();
