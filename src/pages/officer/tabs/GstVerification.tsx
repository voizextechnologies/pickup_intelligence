...
const GstVerification: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<GstVerificationResult | null>(null);
  ...
  const handleGstVerification = async () => {
    ...
    const gstAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('gst verification') && api.key_status === 'Active'
    );

    if (!gstAPI) {
      toast.error('GST Verification API not configured. Please contact admin.');
      setSearchError('GST Verification API not configured');
      return;
    }

    const creditCost = gstAPI.default_credit_charge || 1.50;
    ...
    try {
      ...
      const baseUrl = '/api/planapi/api/Ekyc/GSTVerification';
      ...
      const data: GstVerificationResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `GST Verification query for ${cleanGstNumber}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          source: 'RapidAPI',
          result_summary: `GST Verification for ${cleanGstNumber}: ${data.status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.status === 'Success') {
        ...
        toast.error(`GST verification failed: ${data.msg || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
          ...
          >
            {isSearching ? (
              ...
              <>
                ...
                <span>Check GST Verification</span>
              </>
            )}
          </button>
        </div>
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('gst verification'))?.default_credit_charge || 1.50} credits per query.
      </p>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `gst-verification-${gstNumber}-${Date.now()}.json`;
                link.click();
