...
import { Shield, AlertCircle, CheckCircle, ChevronDown, ChevronUp, Copy, Download, Search, Car } from 'lucide-react';
import { useTheme } from '../../../contexts/ThemeContext';
import { useOfficerAuth } from '../../../contexts/OfficerAuthContext';
import { useSupabaseData } from '../../../hooks/useSupabaseData';
import toast from 'react-hot-toast';

interface VehicleChallanResult {
  [key: string]: any;
}

const VehicleChallanDetails: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<VehicleChallanResult | null>(null);
  ...
  const handleVehicleChallan = async () => {
    if (!vehicleNumber.trim() || !chassisNumber.trim() || !engineNumber.trim()) {
      toast.error('Please enter all required fields: Vehicle Number, Chassis Number, and Engine Number');
      return;
    }
    if (!/^[A-Z]{2}[0-9]{2}[A-Z]{2}\d{4}$/.test(vehicleNumber)) {
      toast.error('Please enter a valid Vehicle Number (e.g., MH01AB1234)');
      return;
    }
    ...
    const vehicleAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('vehicle challan details') && api.key_status === 'Active'
    );

    if (!vehicleAPI) {
      toast.error('Vehicle Challan Details API not configured. Please contact admin.');
      setSearchError('Vehicle Challan Details API not configured');
      return;
    }

    const creditCost = vehicleAPI.default_credit_charge || 2.00;
    ...
    try {
      const apiParts = vehicleAPI.api_key.split(':');
      ...
      const cleanChassisNumber = chassisNumber.trim();
      const cleanEngineNumber = engineNumber.trim();
      const baseUrl = '/api/planapi/api/VehicleChallan';

      const payload = {
        vehicleNumber: cleanVehicleNumber,
        chassisNumber: cleanChassisNumber,
        engine_no: cleanEngineNumber,
        ApiMode: '1',
      };
      ...
      const data: VehicleChallanResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Vehicle Challan query for ${cleanVehicleNumber}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'Vehicle Challan Details',
          input_data: `Vehicle Number: ${cleanVehicleNumber}, Chassis Number: ${cleanChassisNumber}, Engine Number: ${cleanEngineNumber}, ApiMode: 1`,
          source: 'RapidAPI',
          result_summary: `Vehicle Challan for ${cleanVehicleNumber}: ${data.Status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.Status === 'Success') {
        toast.success('Vehicle challan details retrieved successfully!');
      } else {
        toast.error(`Vehicle challan check failed: ${data.Message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Vehicle Challan error:', error);
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'Vehicle Challan Details',
          input_data: `Vehicle Number: ${vehicleNumber}, Chassis Number: ${chassisNumber}, Engine Number: ${engineNumber}, ApiMode: 1`,
          source: 'RapidAPI',
    ...
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="flex items-end">
          <button
            onClick={handleVehicleChallan}
            disabled={isSearching || !vehicleNumber.trim() || !chassisNumber.trim() || !engineNumber.trim()}
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('vehicle challan details'))?.default_credit_charge || 2.00} credits per query.
      </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.Response?.vehicleId || vehicleNumber || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                  ...
                  {searchResults.Response?.Challans && searchResults.Response.Challans.length > 0 && (
                    <>
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Challan Number:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {searchResults.Response.Challans[0].challan_no || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Date:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {searchResults.Response.Challans[0].date || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Amount:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {searchResults.Response.Challans[0].amount || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Area:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {searchResults.Response.Challans[0].area || 'N/A'}
                        </span>
                      </div>
                    </>
                  )}
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `vehicle-challan-${vehicleNumber}-${Date.now()}.json`;
                link.click();
...
export default VehicleChallanDetails;