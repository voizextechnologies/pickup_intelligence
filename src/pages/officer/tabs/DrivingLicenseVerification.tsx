...
interface DLVerificationResult {
  [key: string]: any;
}

const DrivingLicenseVerification: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<DLVerificationResult | null>(null);
  ...
  const handleDLVerification = async () => {
    if (!dlNumber.trim() || !dateOfBirth.trim()) {
      toast.error('Please enter both DL Number and Date of Birth');
      return;
    }
    if (!/^[A-Z]{2}\d{13}$/.test(dlNumber)) {
      toast.error('Please enter a valid DL Number (e.g., MH12345678901234)');
      return;
    }
    if (!/^\d{2}-\d{2}-\d{4}$/.test(dateOfBirth)) {
      toast.error('Please enter Date of Birth in format DD-MM-YYYY (e.g., 01-12-2000)');
      return;
    }
    ...
    const dlAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('driving license verification') && api.key_status === 'Active'
    );

    if (!dlAPI) {
      toast.error('Driving License Verification API not configured. Please contact admin.');
      setSearchError('Driving License Verification API not configured');
      return;
    }

    const creditCost = dlAPI.default_credit_charge || 1.90;
    ...
    try {
      const apiParts = dlAPI.api_key.split(':');
      ...
      const cleanDateOfBirth = dateOfBirth.trim();
      const baseUrl = '/api/planapi/api/Ekyc/DLVerification';

      const payload = {
        DLNumber: cleanDlNumber,
      ...
      const data: DLVerificationResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Driving License Verification query for ${cleanDlNumber}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'Driving License Verification',
          input_data: `DL Number: ${cleanDlNumber}, Date of Birth: ${cleanDateOfBirth}, ApiMode: 1`,
          source: 'RapidAPI',
          result_summary: `Driving License Verification for ${cleanDlNumber}: ${data.status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.status === 'Success') {
        toast.success('Driving License verification successful!');
      } else {
        toast.error(`Driving License verification failed: ${data.msg || 'Unknown error'}`);
      }
    } catch (error) {
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'Driving License Verification',
          input_data: `DL Number: ${dlNumber}, Date of Birth: ${dateOfBirth}, ApiMode: 1`,
          source: 'RapidAPI',
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <input
            ...
            placeholder="Enter DL Number (e.g., MH12345678901234)"
        ...
        <div>
          ...
          <input
            ...
            placeholder="Enter Date of Birth (e.g., 01-12-2000)"
            className={`w-full px-4 py-3 border border-cyber-teal/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyber-teal ${isDark ? 'bg-crisp-black text-white placeholder-gray-500' : 'bg-white text-gray-900 placeholder-gray-400'}`}
          />
        </div>
        <div className="flex items-end col-span-2">
          <button
            ...
            disabled={isSearching || !dlNumber.trim() || !dateOfBirth.trim()}
          ...
          >
            {isSearching ? (
              ...
              <>
                ...
                <span>Check Driving License</span>
              </>
            )}
          </button>
        </div>
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('driving license verification'))?.default_credit_charge || 1.90} credits per query.
      </p>
              ...
              <div className={`p-4 mt-2 rounded-lg border ${isDark ? 'bg-gray-700/50 border-cyber-teal/10' : 'bg-white border-gray-200'}`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>DL Number:</span>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `dl-verification-${dlNumber}-${Date.now()}.json`;
                link.click();
