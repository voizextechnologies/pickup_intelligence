...
interface MCACompanyResult {
  [key: string]: any;
}

const MCACompanySearch: React.FC = () => {
  const { isDark } = useTheme();
  const { officer, updateOfficerState } = useOfficerAuth();
  const { apis, addQuery, addTransaction, getOfficerEnabledAPIs } = useSupabaseData();
  const [companyName, setCompanyName] = useState('');
  const [isSearching, setIsSearching] = useState(false);
  const [searchResults, setSearchResults] = useState<MCACompanyResult | null>(null);
  ...
  const handleMCACompanySearch = async () => {
    if (!companyName.trim()) {
    ...
    const mcaAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('mca company') && api.key_status === 'Active'
    );

    if (!mcaAPI) {
      toast.error('MCA Company API not configured. Please contact admin.');
      setSearchError('MCA Company API not configured');
      return;
    }

    const creditCost = mcaAPI.default_credit_charge || 1.95;
    ...
    try {
      ...
      const baseUrl = '/api/planapi/api/Ekyc/MCACompanySearch';
      ...
      const data: MCACompanyResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `MCA Company Search query for ${cleanCompanyName}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'MCA Company Search',
          input_data: `Company Name: ${cleanCompanyName}, ApiMode: 1`,
          source: 'RapidAPI',
          result_summary: `MCA Company Search for ${cleanCompanyName}: ${data.status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.status === 'Success') {
        toast.success('MCA Company search successful!');
      } else {
        toast.error(`MCA Company search failed: ${data.msg || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('MCA Company Search error:', error);
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'MCA Company Search',
          input_data: `Company Name: ${companyName}, ApiMode: 1`,
          source: 'RapidAPI',
    ...
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="flex items-end">
          <button
            onClick={handleMCACompanySearch}
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('mca company'))?.default_credit_charge || 1.95} credits per query.
      </p>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `mca-search-${companyName}-${Date.now()}.json`;
                link.click();
...
export default MCACompanySearch;