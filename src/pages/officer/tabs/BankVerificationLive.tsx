...
import { Shield, AlertCircle, CheckCircle, ChevronDown, ChevronUp, Download, Search, Banknote } from 'lucide-react';
import { useTheme } from '../../../contexts/ThemeContext';
import { useOfficerAuth } from '../../../contexts/OfficerAuthContext';
import { useSupabaseData } from '../../../hooks/useSupabaseData';
import toast from 'react-hot-toast';

interface BankVerificationLiveResult {
  [key: string]: any;
}

const BankVerificationLive: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<BankVerificationLiveResult | null>(null);
  ...
  const handleBankVerification = async () => {
    ...
    const bankAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('bank verification live') && api.key_status === 'Active'
    );

    if (!bankAPI) {
      toast.error('Bank Verification Live API not configured. Please contact admin.');
      setSearchError('Bank Verification Live API not configured');
      return;
    }

    const creditCost = bankAPI.default_credit_charge || 2.50;
    ...
    try {
      const apiParts = bankAPI.api_key.split(':');
      ...
      const baseUrl = '/api/planapi/api/Ekyc/BankVarificationLive';
      ...
      const data: BankVerificationLiveResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Bank Verification Live query for ${cleanData.AccountNo}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'Bank Verification Live',
          input_data: `Name: ${cleanData.Name}, AccountNo: ${cleanData.AccountNo}, IFSC: ${cleanData.Ifsc}, ApiMode: ${cleanData.ApiMode}`,
          source: 'RapidAPI',
          result_summary: `Bank Verification Live for ${cleanData.AccountNo}: ${data.status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.status === 'Success') {
        ...
        toast.error(`Bank verification failed: ${data.msg || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Bank Verification Live error:', error);
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'Bank Verification Live',
          input_data: `Name: ${name}, AccountNo: ${accountNo}, IFSC: ${ifsc}, ApiMode: ${apiMode}`,
          source: 'RapidAPI',
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('bank verification live'))?.default_credit_charge || 2.50} credits per query.
      </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.response?.beneficiary_name || name || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Account No:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.response?.beneficiary_account || accountNo || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>IFSC:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.response?.beneficiary_ifsc || ifsc || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Account Status:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.response?.account_status || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Bank Name:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.response?.bank_name || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Branch Name:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.response?.branch_name || 'N/A'}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `bank-verification-live-${accountNo}-${Date.now()}.json`;
                link.click();
...
export default BankVerificationLive;