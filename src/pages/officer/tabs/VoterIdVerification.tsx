...
const VoterIdVerification: React.FC = () => {
  const { isDark } = useTheme();
  const { officer, updateOfficerState } = useOfficerAuth();
  const { apis, addQuery, addTransaction, getOfficerEnabledAPIs } = useSupabaseData();
  const [voterId, setVoterId] = useState('');
  const [isSearching, setIsSearching] = useState(false);
  const [searchResults, setSearchResults] = useState<VoterIdVerificationResult | null>(null);
  ...
  const handleVoterIdVerification = async () => {
    ...
    const voterAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('voter id verification') && api.key_status === 'Active'
    );

    if (!voterAPI) {
      toast.error('Voter ID Verification API not configured. Please contact admin.');
      setSearchError('Voter ID Verification API not configured');
      return;
    }

    const creditCost = voterAPI.default_credit_charge || 1.75;
    ...
    try {
      ...
      const baseUrl = '/api/planapi/api/Ekyc/VoterIdVerification';
      ...
      const data: VoterIdVerificationResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Voter ID Verification query for ${cleanVoterId}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'Voter ID Verification',
          input_data: `Voter ID: ${cleanVoterId}, ApiMode: 1`,
          source: 'RapidAPI',
          result_summary: `Voter ID Verification for ${cleanVoterId}: ${data.status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.status === 'Success') {
        toast.success('Voter ID verification successful!');
      } else {
        toast.error(`Voter ID verification failed: ${data.msg || 'Unknown error'}`);
      }
    } catch (error) {
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'Voter ID Verification',
          input_data: `Voter ID: ${voterId}, ApiMode: 1`,
          source: 'RapidAPI',
    ...
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <Identification className="w-6 h-6 text-electric-blue" />
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('voter id verification'))?.default_credit_charge || 1.75} credits per query.
      </p>
              ...
              <div className={`p-4 mt-2 rounded-lg border ${isDark ? 'bg-gray-700/50 border-cyber-teal/10' : 'bg-white border-gray-200'}`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Voter ID:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.response?.epic_no || voterId || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                  ...
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Holder Name:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.response?.holder_name || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
