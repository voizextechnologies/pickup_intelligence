...
const PanByGstNumber: React.FC = () => {
  ...
  const handlePanByGst = async () => {
    ...
    if (!panAPI) {
      ...
      setSearchError('PAN by GST API not configured');
      return;
    }
    ...
    try {
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `PAN by GST query for ${cleanGstNumber}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'PAN by GST',
          input_data: `GST: ${cleanGstNumber}`,
          source: 'PlanAPI',
          result_summary: `PAN by GST for ${cleanGstNumber}`,
          full_result: data,
      ...
      toast.success('PAN by GST retrieved successfully!');
    } catch (error) {
      console.error('PAN by GST error:', error);
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('pan by gst'))?.default_credit_charge || 5} credits per query.
      </p>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `pan-by-gst-${gstNumber}-${Date.now()}.json`;
                link.click();
