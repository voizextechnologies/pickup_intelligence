...
const BankVerification: React.FC = () => {
  ...
  const handleBankVerification = async () => {
    ...
    const bankAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('bank verification') && api.key_status === 'Active'
    );

    if (!bankAPI) {
      toast.error('Bank Verification API not configured. Please contact admin.');
      setSearchError('Bank Verification API not configured');
      return;
    }

    const creditCost = bankAPI.default_credit_charge || 2.25;
    ...
    try {
      ...
      const data: BankVerificationResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Bank Verification query for ${cleanData.AccountNo}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          source: 'RapidAPI',
          result_summary: `Bank Verification for ${cleanData.AccountNo}: ${data.status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('bank verification'))?.default_credit_charge || 2.25} credits per query.
      </p>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `bank-verification-${accountNo}-${Date.now()}.json`;
                link.click();
