...
import { Shield, AlertCircle, CheckCircle, ChevronDown, ChevronUp, Copy, Download, Search, User } from 'lucide-react';
import { useTheme } from '../../../contexts/ThemeContext';
import { useOfficerAuth } from '../../../contexts/OfficerAuthContext';
import { useSupabaseData } from '../../../hooks/useSupabaseData';
import toast from 'react-hot-toast';

interface DINVerificationResult {
  [key: string]: any;
}

const DINVerification: React.FC = () => {
  const { isDark } = useTheme();
  const { officer, updateOfficerState } = useOfficerAuth();
  const { apis, addQuery, addTransaction, getOfficerEnabledAPIs } = useSupabaseData();
  const [din, setDin] = useState('');
  const [isSearching, setIsSearching] = useState(false);
  const [searchResults, setSearchResults] = useState<DINVerificationResult | null>(null);
  ...
  const handleDINVerification = async () => {
    if (!din.trim()) {
      toast.error('Please enter a DIN');
      return;
    }
    if (!/^\d{8}$/.test(din)) {
      toast.error('Please enter a valid DIN (8 digits)');
      return;
    }
    ...
    const dinAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('din verification') && api.key_status === 'Active'
    );

    if (!dinAPI) {
      toast.error('DIN Verification API not configured. Please contact admin.');
      setSearchError('DIN Verification API not configured');
      return;
    }

    const creditCost = dinAPI.default_credit_charge || 1.75;
    ...
    try {
      const apiParts = dinAPI.api_key.split(':');
      ...
      const baseUrl = '/api/planapi/api/Ekyc/DINDetails';
      ...
      const data: DINVerificationResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `DIN Verification query for ${cleanDin}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'DIN Verification',
          input_data: `DIN: ${cleanDin}, ApiMode: 1`,
          source: 'RapidAPI',
          result_summary: `DIN Verification for ${cleanDin}: ${data.status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.status === 'Success') {
        toast.success('DIN verification successful!');
      } else {
        toast.error(`DIN verification failed: ${data.msg || 'Unknown error'}`);
      }
    } catch (error) {
    ...
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="flex items-end">
          <button
            onClick={handleDINVerification}
            disabled={isSearching || !din.trim()}
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('din verification'))?.default_credit_charge || 1.75} credits per query.
      </p>
              ...
              <div className={`p-4 mt-2 rounded-lg border ${isDark ? 'bg-gray-700/50 border-cyber-teal/10' : 'bg-white border-gray-200'}`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>DIN:</span>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `din-verification-${din}-${Date.now()}.json`;
                link.click();
