...
const UpiVerification: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<UpiVerificationResult | null>(null);
  ...
  const handleUpiVerification = async () => {
    ...
    if (!upiAPI) {
      ...
      setSearchError('UPI Verification API not configured');
      return;
    }
    ...
    try {
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `UPI Verification query for ${cleanUpiId}`,
        });
      }
