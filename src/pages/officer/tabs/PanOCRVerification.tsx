...
const PanOCRVerification: React.FC = () => {
  ...
  const handlePanOCR = async () => {
    if (!panImage) {
      toast.error('Please upload a PAN image');
      return;
    }
    ...
    const panAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('pan ocr') && api.key_status === 'Active'
    );

    if (!panAPI) {
      toast.error('PAN OCR API not configured. Please contact admin.');
      setSearchError('PAN OCR API not configured');
      return;
    }

    const creditCost = panAPI.default_credit_charge || 2.00;
    ...
    try {
      const apiParts = panAPI.api_key.split(':');
      ...
      const baseUrl = '/api/planapi/api/Ekyc/PANOCR';
      ...
      const data: PanOCRResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `PAN OCR query for ${panImage.name}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'PAN OCR Verification',
          input_data: `Pan Image: ${panImage.name}, ApiMode: 1`,
          source: 'RapidAPI',
          result_summary: `PAN OCR for ${panImage.name}: ${data.Status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.Status === 'Success') {
        toast.success('PAN OCR verification successful!');
      } else {
        toast.error(`PAN OCR verification failed: ${data.Message || 'Unknown error'}`);
      }
    } catch (error) {
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'PAN OCR Verification',
          input_data: `Pan Image: ${panImage?.name || 'N/A'}, ApiMode: 1`,
          source: 'RapidAPI',
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('pan ocr'))?.default_credit_charge || 2.00} credits per query.
      </p>
            ...
            {expandedSections.details && (
              <div className={`p-4 mt-2 rounded-lg border ${isDark ? 'bg-gray-700/50 border-cyber-teal/10' : 'bg-white border-gray-200'}`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Father's Name:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.data?.FatherName || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `pan-ocr-${Date.now()}.json`;
                link.click();
...
export default PanOCRVerification;