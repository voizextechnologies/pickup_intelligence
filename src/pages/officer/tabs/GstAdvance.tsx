...
interface GstAdvanceResult {
  [key: string]: any;
}

const GstAdvance: React.FC = () => {
  const { isDark } = useTheme();
  const { officer, updateOfficerState } = useOfficerAuth();
  const { apis, addQuery, addTransaction, getOfficerEnabledAPIs } = useSupabaseData();
  const [gstNumber, setGstNumber] = useState('');
  const [isSearching, setIsSearching] = useState(false);
  const [searchResults, setSearchResults] = useState<GstAdvanceResult | null>(null);
  ...
  const handleGstAdvance = async () => {
    ...
    const gstAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('gst advance') && api.key_status === 'Active'
    );

    if (!gstAPI) {
      toast.error('GST Advance API not configured. Please contact admin.');
      setSearchError('GST Advance API not configured');
      return;
    }

    const creditCost = gstAPI.default_credit_charge || 2.00;
    ...
    try {
      ...
      const baseUrl = '/api/planapi/api/Ekyc/GstDetailsAndVerify';
      ...
      const data: GstAdvanceResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `GST Advance query for ${cleanGstNumber}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'GST Advance',
          input_data: `GST Number: ${cleanGstNumber}, ApiMode: 1`,
          source: 'RapidAPI',
          result_summary: `GST Advance for ${cleanGstNumber}: ${data.Status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.Status === 'Success') {
        toast.success('GST advance check successful!');
      } else {
        toast.error(`GST advance check failed: ${data.Message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('GST Advance error:', error);
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('gst advance'))?.default_credit_charge || 2.00} credits per query.
      </p>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `gst-advance-${gstNumber}-${Date.now()}.json`;
                link.click();
