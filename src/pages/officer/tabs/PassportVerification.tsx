...
const PassportVerification: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<PassportVerificationResult | null>(null);
  ...
  const handlePassportVerification = async () => {
    if (!fileNo.trim() || !dateOfBirth.trim()) {
      toast.error('Please enter both passport file number and date of birth');
      return;
    }
...
const passportAPI = enabledAPIs.find(api =>
  api.name.toLowerCase().includes('passport verification') && api.key_status === 'Active'
    ...
    if (!passportAPI) {
      ...
      setSearchError('Passport Verification API not configured');
      return;
    }

    const creditCost = passportAPI.default_credit_charge || 5;
    ...
    try {
      const [apiUserId, apiPassword, tokenId] = passportAPI.api_key.split(':');
...
const response = await fetch(proxyUrl, {
  ...
  body: JSON.stringify(payload),
      ...
      const data: PassportVerificationResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Passport Verification query for ${cleanFileNo}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          input_data: `File No: ${cleanFileNo}, DOB: ${cleanDateOfBirth}`,
          source: 'PlanAPI',
          result_summary: `Passport verification for ${cleanFileNo}`,
          full_result: data,
      ...
      toast.success('Passport verification completed successfully!');
    } catch (error) {
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'Passport Verification',
          input_data: `File No: ${fileNo}, DOB: ${dateOfBirth}`,
          source: 'PlanAPI',
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <input
            ...
            placeholder="Enter passport file number (e.g., Z1234567)"
        ...
        <div>
          ...
          <input
            ...
            placeholder="Enter DOB (e.g., 01-12-2000)"
            className={`w-full px-4 py-3 border border-cyber-teal/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyber-teal ${isDark ? 'bg-crisp-black text-white placeholder-gray-500' : 'bg-white text-gray-900 placeholder-gray-400'}`}
          />
        </div>
        <div className="flex items-end md:col-span-2">
          <button
            ...
            disabled={isSearching || !fileNo.trim() || !dateOfBirth.trim()}
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('passport verification'))?.default_credit_charge || 5} credits per query.
      </p>
              ...
              <div className={`p-4 mt-2 rounded-lg border ${isDark ? 'bg-gray-700/50 border-cyber-teal/10' : 'bg-white border-gray-200'}`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>File Number:</span>
                    <div className="flex items-center space-x-2">
                      <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        {searchResults.fileNo || 'N/A'}
                      </span>
                      {searchResults.fileNo && (
                        <button
                          onClick={() => copyToClipboard(searchResults.fileNo)}
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `passport-verification-${fileNo}-${Date.now()}.json`;
                link.click();
