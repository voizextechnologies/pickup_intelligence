...
import { Shield, AlertCircle, CheckCircle, ChevronDown, ChevronUp, Copy, Download, Search, CarFront } from 'lucide-react';
import { useTheme } from '../../../contexts/ThemeContext';
import { useOfficerAuth } from '../../../contexts/OfficerAuthContext';
import { useSupabaseData } from '../../../hooks/useSupabaseData';
import toast from 'react-hot-toast';

interface RCVerificationResult {
  [key: string]: any;
}

const RegistrationCertificateVerification: React.FC = () => {
  const { isDark } = useTheme();
  const { officer, updateOfficerState } = useOfficerAuth();
  const { apis, addQuery, addTransaction, getOfficerEnabledAPIs } = useSupabaseData();
  const [vehicleNumber, setVehicleNumber] = useState('');
  const [isSearching, setIsSearching] = useState(false);
  const [searchResults, setSearchResults] = useState<RCVerificationResult | null>(null);
  ...
  const handleRCVerification = async () => {
    ...
    const rcAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('registration certificate') && api.key_status === 'Active'
    );

    if (!rcAPI) {
      toast.error('Registration Certificate API not configured. Please contact admin.');
      setSearchError('Registration Certificate API not configured');
      return;
    }

    const creditCost = rcAPI.default_credit_charge || 1.85;
    ...
    try {
      const apiParts = rcAPI.api_key.split(':');
      ...
      const baseUrl = '/api/planapi/api/Ekyc/RC_Verification';
      ...
      const data: RCVerificationResult = await response.json();
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `RC Verification query for ${cleanVehicleNumber}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'Registration Certificate Verification',
          input_data: `Vehicle Number: ${cleanVehicleNumber}, ApiMode: 1`,
          source: 'RapidAPI',
          result_summary: `RC Verification for ${cleanVehicleNumber}: ${data.status === 'Success' ? 'Successful' : 'Failed'}`,
          full_result: data,
      ...
      if (data.status === 'Success') {
        toast.success('Registration Certificate verification successful!');
      } else {
        toast.error(`Registration Certificate verification failed: ${data.msg || 'Unknown error'}`);
      }
    } catch (error) {
      ...
      if (addQuery) {
        await addQuery({
          ...
          type: 'PRO',
          category: 'Registration Certificate Verification',
          input_data: `Vehicle Number: ${vehicleNumber}, ApiMode: 1`,
          source: 'RapidAPI',
    ...
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <CarFront className="w-6 h-6 text-electric-blue" />
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('registration certificate'))?.default_credit_charge || 1.85} credits per query.
      </p>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `rc-verification-${vehicleNumber}-${Date.now()}.json`;
                link.click();
...
export default RegistrationCertificateVerification;